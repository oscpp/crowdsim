...
namespace Image
{
    ...
    public:
		...
		// load image header and data from a bmp file, sets alpha channel
	    bool read(const char* fileName, bool alpha);
	...
	public:
		...
		int  getRGBXY(int RGB, int x, int y);		// get the RGB information at pixel XY
		void setRGBXY(int R, int G, int B, int x, int y);// set the RGB information at pixel XY
		void makeRGBA();
