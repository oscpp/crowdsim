...
///////////////////////////////////////////////////////////////////////////////
// read a BMP image header infos and datafile and load as a RGBA file
///////////////////////////////////////////////////////////////////////////////
bool Bmp::read(const char* fileName, bool alpha)
{
	Bmp::read(fileName);
	...
	if (alpha)
	{
	 ...
	 dataRGBA	= new unsigned char [dataSize+(dataSize/3)]; // allocate extra memory space for RGBA.
	 ...
	 for( i = 0, j = 0; i < dataSize; i += 3, j += 4 )
        {
			...
			int g_keyColor[3] = { 0, 255, 0 }; // Pure green is the color key to blend out

            // Does the current pixel match the selected color key?
            if( dataRGB[i]   == g_keyColor[0] &&
                dataRGB[i+1] == g_keyColor[1] &&
                dataRGB[i+2] == g_keyColor[2] )
            {
                dataRGBA[j+3] = 0;   // If so, set alpha to fully transparent.
            }
            else
            {
                dataRGBA[j+3] = 255; // If not, set alpha to fully opaque.
            }

			dataRGBA[j] = dataRGB[i];
			dataRGBA[j+1] = dataRGB[i+1];
			dataRGBA[j+2] = dataRGB[i+2];
		
        }
	 
	 //delete [] dataRGB; //we do not need RGB anymore.
	}
...
///////////////////////////////////////////////////////////////////////////////
// returns either R, G, B of a given x-y position
///////////////////////////////////////////////////////////////////////////////
int Bmp::getRGBXY(int RGB, int x, int y)
{
	int i = (getWidth() * (y * 3)) + (x * 3);

	if (RGB == 1) { return dataRGB[i+0]; }
	if (RGB == 2) { return dataRGB[i+1]; }
	if (RGB == 3) { return dataRGB[i+2]; }
	return NULL;
}

///////////////////////////////////////////////////////////////////////////////
// set the RGB information at pixel XYZ. Does not modify the file, 
// only what is loaded in memory
///////////////////////////////////////////////////////////////////////////////
void Bmp::setRGBXY(int R, int G, int B, int x, int y)
{
	int i = (getWidth() * (y * 3)) + (x * 3);
	dataRGB[i+0] = R;
	dataRGB[i+1] = G;
	 ...
	 for( i = 0, j = 0; i < dataSize; i += 3, j += 4 )
        {
			...
			int g_keyColor[3] = { 0, 255, 0 }; // 0 255 0 is the color key to blend out
